...
def main():
    ...
    with open(sheet, "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["model", "scenario", "rep", "decision", "schema_valid",
                    "viability_budget", "expected", "candidate_xml",
                    "grader1_class", "grader2_class", "grader_notes"])
        for r in records:
            w.writerow([r["model"], r["scenario"], r["rep"], r.get("decision"),
                        r.get("_schema_valid"), r.get("viability_budget"),
                        r.get("expected"),
