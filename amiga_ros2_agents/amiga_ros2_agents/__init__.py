...
``runtime`` knows nothing about missions. ``mission``, and every module in
``verification`` except ``ltl_gen_node``, hold no rclpy at all -- they are
libraries, unit-testable with no ROS and no network, which is what lets the
verification claim be checked on its own. When something needs to reach
sideways -- the arbiter needs the gate -- it imports the library, never the
other agent, because agents coordinate over topics and services.
