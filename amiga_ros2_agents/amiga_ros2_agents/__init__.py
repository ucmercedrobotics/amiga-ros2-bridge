...
One package, four directories, and the split between them is what each part is
allowed to know:
...
``runtime`` knows nothing about missions. ``mission`` holds no rclpy at all --
it is a library, unit-testable with no ROS and no network. When something
needs to reach sideways -- the arbiter needs the ontology -- it imports the
library, never the other agent, because agents coordinate over topics and
services.
