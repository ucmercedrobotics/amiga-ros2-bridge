...
thread-safe RobotContext. Also serves a lightweight A2A streaming
endpoint on port 10004.
"""
...
class WorldStateNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("WorldStateNode started")
    ...
    def _on_segment_status(self, msg):
        with self._lock:
            if msg.status_list:
                self._state.segment_status = self._status_str(msg.status_list[-1].status)
