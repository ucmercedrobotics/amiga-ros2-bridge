...
BT::NodeStatus MoveToGPSLocation::onResultReceived(const WrappedResult &result)
{
    RCLCPP_INFO(logger(), "Navigation finished successfully: %d", int(result.code));
    return BT::NodeStatus::SUCCESS;
}

BT::NodeStatus MoveToGPSLocation::onFeedback(const std::shared_ptr<const Feedback> feedback)
{
